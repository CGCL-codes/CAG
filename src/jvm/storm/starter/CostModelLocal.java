...
/* 65 */       Integer bolt_counts = Integer.valueOf(args[2]);
               ...
               Double skew = Double.valueOf(args[9]); 
                ...
                String ds = args[11]; 
                if (is_aggregation_step.intValue() == 0  && skew>0 ){
                    ...
                    builder.setBolt("call-log-counter-bolt", new DataCounterBolt(args[0], spouts, bolt_counts,bolt_agg, is_aggregation_step, tuples_to_emit,skew,ackers), bolt_counts)
                            .localOrShuffleGrouping("kafka-spout");
                    builder.setBolt("call-log-agg-bolt", new MyAggregator(args[0], spouts, bolt_counts,bolt_agg, is_aggregation_step, tuples_to_emit,ackers), bolt_agg)
                            .localOrShuffleGrouping("call-log-counter-bolt");
                            //.fieldsGrouping("call-log-counter-bolt", new Fields(new String[] { "word" }));

//                      builder.setBolt("call-log-creator-bolt", new DataCreatorBolt(), bolt_words).fieldsGrouping("kafka-spout", new Fields(new String[] { "word" }));
//                    builder.setBolt("call-log-counter-bolt", new DataCounterBolt(args[0], spouts, bolt_words, bolt_counts, is_aggregation_step, tuples_to_emit,bolt_agg,skew), bolt_counts).fieldsGrouping("call-log-creator-bolt", new Fields(new String[] { "word" }));
//                    builder.setBolt("call-log-agg-bolt", new MyAggregator(args[0], spouts, bolt_words, bolt_counts, is_aggregation_step, tuples_to_emit,bolt_agg), bolt_agg).fieldsGrouping("call-log-counter-bolt", new Fields(new String[] { "word" }));
...
/* 90 */         config.setNumWorkers(number_of_workers.intValue());
